...
def talk(text):
    args = args = audio_args(language='en',output=open('/tmp/test.mp3', 'w')) 
    audio_extract(input_text=text, args=args)
    subprocess.Popen(['vlc', '/tmp/test.mp3', '--play-and-exit', '--intf','dummy']).wait()
    

if __name__ == "__main__":
    talk('get your own fucking beer')
